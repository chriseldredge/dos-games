...
VBINT scpy(BINT8 *to, BINT8 *from, NOBINT32 w)
{
...
VBINT tcpy(BINT8 *to, BINT8 *from, NOBINT32 w)
{
...
VBINT tscpy(BINT8 *to, BINT8 *from, NOBINT32 w)
{
...
VBINT rcpy(BINT8 *to, BINT8 *from, NOBINT32 wi)
{
 NOBINT32 w;                              
 VBINT asmcpy(VBINT);
 #pragma aux asmcpy =           \
...
void SVGA_SavePCX(STR *fn, IMAGE_PTR img)
{
 ...
 pcx.reserved = 0;
 pcx.colour_planes = 1;
 if (BytesPerPixel>1) pcx.colour_planes = 3;
     ...
     for (i=0;i<3;i++);
      {
       for (x=0;x<img->width;)
        {
         bpr = 0;
         c = img->hMem[(y*img->width+x)*4+i];
/*         if (img->hMem[(y*img->width+x)*4+4+i]==c)
          {
           bpr = 1;
           x++;
           while (img->hMem[(y*img->width+x)*4+4+i]==c)
            {
             x++; bpr++;
             if ((bpr + 192) > 254) break;
             if (x>img->width) break;
   ...
   for (x1=0;x1<sw*2;x1++)
    {
     ...
     if ((xval<img->width)&&(yval<img->height)&&(xval>=0)&&(yval>=0))
      {
       if (img->hMem[xval+yval*img->width])
...
VBINT _loadds _saveregs SVGA_SetPalette(NOBINT8 index, RGB_PTR color)
{
...
VBINT _loadds _saveregs SVGA_GetPalette(NOBINT8 index, RGB_PTR color)
{
 outp(PALETTE_MASK,0xff);
